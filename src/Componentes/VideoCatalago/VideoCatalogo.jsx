...
const VideoCatalogo = ({ curso }) => {
  // curso é passado como prop
  const [selectedAulaIndex, setSelectedAulaIndex] = useState(0);
  const aula = curso.aulas[selectedAulaIndex]; // usa aulas do curso passado como prop
      ...
      <div className="sidebar">
        <div className="espacamento">
          <img
            width="70"
            height="50"
            src={CodeAcademyLogoSemFundoSemLegenda}
            alt=""
          />
          <h1>Code Academy</h1>
        </div>
        ...
        <ul>
          {curso.aulas.map((aula, index) => (
            <li key={index}>
              <button onClick={() => handleAulaChange(index)}>
                {aula.title}
              </button>
            </li>
          ))}
